...
- The workflow half of the guard is an allowlist too, which took two review rounds to become true. `.github/workflows/codeql.yml` is checked for the `config-file:` wiring, the `security-extended,security-and-quality` suite, the language matrix, the `languages: ${{ matrix.language }}` binding that makes the matrix mean anything, an exact ordered list of every action it runs, the matrix's own key set, and the complete ordered input set of each `codeql-action` step. Round one guarded the workflow with a denylist — the one thing the brief forbade — and deleting the `Perform CodeQL Analysis` step, commenting it out, hardcoding `languages:` past the matrix, and repointing an action at a fork all passed. Round two pinned the actions and the binding, and the same shape reappeared one level down: `upload: never` added to the analyze step (a real action input that suppresses the results upload — green job, no alerts, ever) and `exclude:` added under `matrix:` (drops a language from the expansion while `language:` still lists it) both passed, as did an inserted step whose `uses:` carried a trailing comment, `if : false` with a space before the colon, and a decoy `run:` block that misdirected the non-global assertions onto text that was not the setting. Pinning whole key sets rather than the keys somebody thought of is what closed the class; every one of those eleven mutations is now a case.
...
- `tests/unit/checkCodeqlConfig.test.js` is the behaviour half, because a structural assertion cannot catch a hollowed-out gate and smoke step 16 runs the guard against exactly one input: the one it passes on. Its 65 cases execute the checker against mutated fixtures — 11 config mutations, 21 workflow mutations, 8 benign reformats plus 1 prose control as negative controls, 10 shapes the parser must refuse, 9 covering the real repo and reads from temp directories on disk, and 5 on the parser itself. Every mutation case first asserts the mutation actually changed something; the first run caught one that had silently no-op'd against a CRLF checkout and was asserting against a pristine config. Seventeen hollow-outs of the guard are verified red against a green control — ten of them found by review rather than by the author, across two rounds. The sharpest were not the obvious `return []` but the self-agreeing ones: a constant redefined to derive from the very file it guards makes its gate agree with itself forever, and `checkRepo` ignoring its root argument for one of the two files it reads leaves that entire half decorative while every case stays green. Pinning six literals one at a time would have left the seventh, so the test asserts the property instead — nothing above the checks divider may touch the filesystem at all.
