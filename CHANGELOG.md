...
- A fixture-contract assertion compared a hardcoded date literal against a value derived from `Date.now()`, and turned `main` red on 2026-08-27 at 08:00Z. `nextHalving()` projects forward from the current time, and for the fixture height of 900000 the offset is `150000/144` = 1041 days + exactly 16 hours - so the projected calendar date rolls over at 08:00Z every day while the literal `2029-07-03` stays put. That exact-16-hour offset is why it read as a flake: for the last eight hours before the boundary the same commit passed or failed by what time of day it ran. It does not recover, it drifts a further day daily. Nothing caught it because main last ran CI at 07:49Z and went green eleven minutes before the flip - `build` runs `npm run test:unit` and is a required check, so every branch was blocked from that point. `Date.now()` is now pinned for that one assertion to the instant the literal was authored, which leaves the fixture file unchanged; re-pinning the literal to the current date was the tempting one-character fix and would have re-armed the same failure for the next day. Mutation-verified in three directions - a wrong fixture value, a removed pin, and a pin moved by one day all fail - so the assertion still catches what it was written to catch. It was the only assertion in the suite comparing a hardcoded absolute against a clock-derived value; the neighbouring checks are relative or bounded.
...
- The `smoke` job in `.github/workflows/docker.yml` is now guarded. It is the only thing in the repo that asserts the shipped image works, and three separate PRs had already fixed vacuity bugs in it — #144 (both page assertions grepped `<!doctype html>`, which both pages emit, so the dashboard had never once been verified), #146 (`curl` piped into `python3`, and the job shell has errexit but not pipefail, so the failing curl was discarded), and #148 (`> /tmp/probe.py` truncated the file before the extraction pipeline ran, and `python3 - < empty-file` exits 0). Every one of those fixes was protected by nothing but a comment, including a literal "Do NOT collapse this back into a single pipeline" — `grep -rln docker.yml tests/ scripts/` returned nothing. A comment-asserted invariant is a TODO, not a guard.
- `scripts/check-docker-smoke.cjs` pins the job as a strict allowlist, `deepStrictEqual` throughout: the job list, the smoke job's whole key set (`continue-on-error` makes it advisory while still reporting green, and a `defaults:` block swaps out the very shell the capture assertions depend on), its `if:` and `runs-on`, `on.pull_request.paths` (the gate on whether it runs at all), the ordered step list with each step's complete key set, the build step's `with:`, both page steps' `env:`, and the full comment-stripped body of every `run:` in the job. Comments are stripped before that comparison on purpose — a comment does not execute, and an assertion prose can satisfy is the failure mode being closed — so commenting a line out reads as the deletion it is.
- On top of the pin sits a semantic layer that survives the pin being updated: captures must be BARE assignments (`local x=$(cmd)` and `export x=$(cmd)` do not abort under errexit, because the declaration builtin's own status wins, so those forms are rejected by name), each page step must assert its own marker present AND the other page's marker absent, and the blank-probe check must exist, exit non-zero, and run BEFORE the write and the exec. That ordering is the whole point: a non-blank check placed after `> /tmp/probe.py` still reads as a guard and guards nothing.
- The marker pair is also checked against the real bytes: `fetchBTCPrice` must appear in `index.html` and not in `setup.html`, and `id="ips-msg"` the other way round. #144's real lesson was that a marker can match both the state under test and its opposite, which no structural assertion can see — `dailynode-prefs` looks like an ideal dashboard marker and appears in both pages, so it serves as the negative control for that check rather than as a marker.
- Wired as its own step in `.github/workflows/build.yml`, not only into the `npm test` chain: that workflow enumerates its steps individually and never runs `npm test`, so a check reachable only through the chain gets zero CI coverage — which is what bit #128. `tests/unit/checkDockerSmoke.test.js` is the behaviour half, at 83 cases, and it asserts the wiring by parsing `build.yml` and `package.json` rather than grepping them.
- Verified by mutation rather than by inspection: 39 hollow-outs of the guard, one per pinned property, each required to turn the suite red. The first run had six survivors, and they were the point of running it. The bare-capture check and both exit-non-zero checks turned out to be unreachable — every case reached them through a workflow the line-pin had already rejected, so they would have shipped decorative behind a green suite. A `pinnedRunLines` seam now reproduces the one state where they are the last thing standing (somebody edits a step, sees the guard go red, and dutifully updates the allowlist), and those cases assert both halves: that the pin has genuinely gone quiet, and that the semantic check has not. The document-marker branch of the parser's structural check was deleted rather than kept — `---` and `...` are rejected as unparseable keys either way, so it claimed a protection the anchoring already provided. The tab branch stayed, because a tab inside a scalar value parses cleanly without it, and a case now names that input.
- Known limit, stated rather than implied: this is a structural guard over a workflow file plus two static assertions over the HTML. It proves the smoke job still says what it should. It cannot prove GitHub ran it, that `ubuntu-latest`'s default shell is still `/usr/bin/bash -e {0}`, or that the container behaved — those run on GitHub's side, and no assertion here observes a real job.
