...
- The workflow half of the guard is an allowlist too, which it was not at first. `.github/workflows/codeql.yml` is checked for the `config-file:` wiring, the `security-extended,security-and-quality` suite, the language matrix, and — the part that was missing — an exact ordered list of every action it runs (`EXPECTED_USES`) plus the `languages: ${{ matrix.language }}` binding that makes the matrix mean anything. Review caught that the original version guarded the workflow with a denylist, the one thing the brief forbade, and it showed: deleting the `Perform CodeQL Analysis` step entirely, commenting it out, hardcoding `languages:` past the matrix so Python silently stopped being scanned, and repointing an action at a fork all passed. Each leaves the job named, required, green, and analysing less or nothing — the precise failure the guard claims to prevent. All four are rejected now, with cases covering them.
...
- `tests/unit/checkCodeqlConfig.test.js` is the behaviour half, because a structural assertion cannot catch a hollowed-out gate and smoke step 16 runs the guard against exactly one input: the one it passes on. Its 56 cases execute the checker against mutated fixtures — 11 config mutations, 14 workflow mutations, 8 benign reformats plus 1 prose control as negative controls, 10 shapes the parser must refuse, 7 covering the real repo and reads from a temp directory on disk, and 5 on the parser itself. Every mutation case first asserts the mutation actually changed something; the first run caught one that had silently no-op'd against a CRLF checkout and was asserting against a pristine config. Twelve hollow-outs of the guard are verified red against a green control, including a checker returning `[]` for everything, a parser yielding an empty object for every input, the key-set check weakened from equality to a subset test, and a checker returning a problem for everything — that last one proving the suite is not satisfied by a guard that rejects its own valid input. Three of those twelve were found by review rather than by the author: `checkRepo` hardcoding the config instead of reading disk (which makes `npm run check:codeql` and smoke step 16 decorative), `PATHS_IGNORE_ALLOWED` derived from the very file it guards (self-fulfilling, so any widening is accepted), and the deletion of the workflow allowlist. A fourth was found on the first run — a parser that skipped lines it could not classify kept all 39 cases green, because every "refuses" case reached a specific throw and none reached the generic one.
