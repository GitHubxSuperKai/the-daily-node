...
7. **Enable the secrets hook** — `git config core.hooksPath .githooks`, once per clone. This repo is public, and while CI runs the same scan over your PR diff, the hook catches a leak before it is ever committed rather than after it is pushed. See [docs/SETUP.md](./docs/SETUP.md) step 3 for what it does and does not cover — the scan is narrower than you would expect.
